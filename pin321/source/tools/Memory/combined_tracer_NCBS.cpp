...
static VOID pin_magic_inst(THREADID tid, ADDRINT value, ADDRINT field){
	std::cout<<"in pin_magic_inst, tid: "<<tid<< "PID: "<< curpid <<std::endl;
	//std::cout<<"field value: "<<field<<", value value: "<<value<<std::endl;
    ...
    // curr_instr[tid].msgNode = 99;
        ...
        switch(field){
			...
			case 0x1: //ROI end master
				inROI_master=false;
                std::cout<<"ROI END (MASTER)"<<std::endl;
                break;
...
VOID ThreadStart(THREADID tid, CONTEXT* ctxt, INT32 flags, VOID* v) {
    ...
    tfname << "memtrace_t" <<curpid <<"_" <<  tid << ".out";
    trace[tid] = fopen(tfname.str().c_str(), "wb");
    //trace_sancheck = fopen("asdf.txt", "w");
    ////////open nofilter mtrace
    std::ostringstream mtfname;
    mtfname << "mtrace_t" <<curpid <<"_"  <<tid<<"_"<<0<<".out";
    mtrace_nofilter[tid] = fopen(mtfname.str().c_str(), "wb");
    mt_nf_open[tid]=true;

    if(is_champsim_trace_tid(tid)){
        std::ostringstream ctfname;
        ctfname << "champsim_"<<curpid <<"_" << tid << "_" << 0 << ".trace";
        champsim_outfile[tid].open(ctfname.str().c_str(), std::ios_base::binary | std::ios_base::trunc);
        if (!champsim_outfile[tid])
        {
            std::cout << "Couldn't open output trace file. Exiting." << std::endl;
