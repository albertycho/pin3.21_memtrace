...
#define MT_START 5000000000
#define MT_END   6000000000

BOOL tr_done[MAX_THREADS]={};
...
static VOID InsRef(ADDRINT addr, THREADID tid) //TODO add threaID to arg
{
    ...
    //if(ins_count[tid] % 1000==0){ // for testing
        ...
        if(ins_count[tid] >= MT_END){
            tr_done[tid]=true;
            uint64_t done_count=0;
            for (uint64_t ii=0;ii<MAX_THREADS;ii++){
                if(tr_done[ii]) done_count++;
                else break;
            }
            if(done_count==MAX_THREADS){
                std::cout<<"all threads tracing done"<<std::endl;
                exit(0);
            }
        }
...
static VOID MemRefMulti(ADDRINT addr, UINT32 size, CACHE_BASE::ACCESS_TYPE accessType, THREADID tid) //TODO add threaID to arg
{
    ...
    if(ins_count[tid]<MT_START) return;
    if(ins_count[tid]>MT_END) return;
...
VOID ThreadStart(THREADID tid, CONTEXT* ctxt, INT32 flags, VOID* v) {
	if(tid>=MAX_THREADS){
        std::cout<<"threadid larger than "<<MAX_THREADS<<std::endl;
		return;
	}
