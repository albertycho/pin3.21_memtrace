...
for i, color in enumerate(colors):
    ...
    ax.bar(indices-bar_width*2.6, [base[i] for base in base_values], bar_width, color=color, alpha=0.9, bottom=bottom_values)
...
for i, color in enumerate(colors):
    ...
    ax.bar(indices -bar_width*1.2, [cxi[i] for cxi in cxi_values], bar_width, color=color, hatch='//', alpha=0.6, bottom = cbottom_values)
    cbottom_values = [a + b for a, b in zip(plotvals, cbottom_values)]

ax2 = ax.twinx()
ax2.bar(indices+bar_width*1.2, base_amats, bar_width*0.4, color='black', alpha=1)
ax2.bar(indices+bar_width*2.4, cxi_amats, bar_width*0.4, color='grey', hatch='//', alpha=0.8)
...
ax2.set_ylabel('AMAT(ns)', fontsize=labelfontsize, labelpad=10)
