...
def plot_migration(input_file, output_file):
    ...
    #plt.rcParams.update({'edgecolor':'black'})
    labelfontsize = 20
...
#for i in range(len(DDR_mlats)):
    bars_migrate_32k = plt.bar(index - bar_width * 2, migrate_32k, bar_width, label='32K pages', color=color_list[0], edgecolor='black')
    bars_migrate_8k = plt.bar(index - bar_width, migrate_8k, bar_width, label='8K pages', color=color_list[1], edgecolor='black' )
    bars_migrate_2k = plt.bar(index, migrate_2k, bar_width, label='2K pages', color=color_list[2], edgecolor='black' )
    bars_migrate_512 = plt.bar(index + bar_width, migrate_512, bar_width, label='512 pages', color=color_list[3], edgecolor='black')
    bars_no_migration = plt.bar(index + bar_width * 2, no_migration, bar_width, label='No Migration', color=color_list[4], edgecolor='black')
    ...
    legend_handles = [
        Patch(facecolor=color_list[0], edgecolor='black', label='32K pages'),
        Patch(facecolor=color_list[1], edgecolor='black', label='8K pages'),
        Patch(facecolor=color_list[2], edgecolor='black', label='2K pages'),
        Patch(facecolor=color_list[3], edgecolor='black', label='512 pages'),
        Patch(facecolor=color_list[4], edgecolor='black', label='No Migration')
    ]
    ...
    plt.legend(handles=legend_handles, ncol=2, loc='best', framealpha=0.99, fontsize=14.5)
